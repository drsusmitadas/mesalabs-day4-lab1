...
! ***********************************************************************
    ...
    contains
      ...
      integer function extras_check_model(id)
        ...
        !!! add r integration function here (from Joel)
      ...
      integer function extras_finish_step(id)
        ...
        ! only save profiles when around the target numax and delta nu
        !if ((s% center_he4 > s% center_h1) .and. (s% nu_max < 150) .and. &
         !    (s% nu_max > 5)) then
           ! write profiles or just GYRE files
        !   s% job% save_pulse_data_for_model_number = s% model_number
        !   fmt = '(I0.4)'
        !   write(ind,fmt) s% model_number
        !   s% job% save_pulse_data_filename = 'gyre_LOGS/gyre_profile'//trim(ind)//'.data.GSM'
           !s% write_profiles_flag = .true.
           ! could now change timestep controls or intervals if wanted
        !endif

        
        ! custom stopping condition on AGB rise
        !if (s% center_he4 < 1d-4 .and. s% nu_max < 10) then
        !   extras_finish_step = terminate
        !   write(*,*) 'on AGB branch'
        !end if

        !! add custom stopping condition here
